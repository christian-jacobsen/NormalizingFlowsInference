...
from models.bayesian_layers import BayesianFNN, LinearForwardwAct

class FunctionalInferenceECOAT_MCMC(nn.Module):
    def __init__(self, nodes, layers, act=nn.ReLU()):
        super().__init__()
        self.D = 3 # 3 base parameters to infer

        # input weights W and sample x, output act(Wx)
        self.act_model = LinearForwardwAct(nodes, layers, act=act)

    def forward_model(self, W, L, tFinal, dt, Sigma, R_film0, Cv, K, jmin, model, VR=1, CC=1):
        ...
        for i in range(nt - 1):
            ...
            print('beta: ', beta)
            '''
            Q[:, inds[i+1], 0] = Q[:, inds[i], 0] + current[:, inds[i+1], 0]  # Sigma*BC_anode[inds[i+1]] / (Sigma*resistance[:, inds[i], 0] + L)*dt
            ...
            print('jip1: ', j[:, i+1, 0])
            '''
            thickness[:, inds[i+1], 0] = self.tradeoff(self.limit(thickness[:, inds[i], 0] + Cv.reshape((1, -1))*(current[:, inds[i+1], 0] - jmin.reshape((1, -1)))*dt, 0, 1e9), thickness[:, inds[i], 0], Q[:, inds[i+1], 0], Qmin, 1e9)
            resistance[:, inds[i+1], 0] = self.tradeoff(self.limit(resistance[:, inds[i], 0] + self.rho(W, current[:, inds[i+1], 0].reshape((-1, 1))).reshape((1, -1))*(current[:, inds[i+1], 0]-jmin.reshape((1, -1)))*dt*Cv.reshape((1, -1)), R_film0, 1e9), resistance[:, inds[i], 0], Q[:, inds[i+1], 0], Qmin, 1e9)
    ...
    def rho(self, W, j):
        j = self.act_model(W, j)
        return self.limit(2e6, 8e6*torch.exp(-0.1*j), 1e9)
        #return self.limit(W[:, 0].reshape((-1, 1))*1e6, W[:, 1].reshape((-1, 1))*1e6*torch.exp(-W[:, 2].reshape((-1, 1))*j), 1e9)
    ...
    def forward(self, b, z, W, L, tFinal, dt, Sigma, R_film0, model, VR=1, CC=1):
        thk, res, cur, tv = self.forward_model(W, L, tFinal, dt, Sigma, R_film0,
                                               10**(-z[:, 0]), z[:, 1], z[:, 2],
                                               model, VR=VR, CC=CC)

        return thk[:, :, 0], 1e2*res[:, :, 0], 1e-3*cur[:, :, 0], tv
