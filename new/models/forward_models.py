...
def forward_model_ford_old(L,N,dt,tFinal,Sigma,R_film0, VR, Cv, K, jmin):
...
def forward_model_cc_new(L, N, dt, tFinal, Sigma, R_film0, CurrentAnode, Cv, K, jMin, VoltageMax):
    ...
    while t < tFinal:
        ...
        if (Q > Qmin):
            # film thickness
            h = np.maximum(h + dt * Cv * (j - jMin), 0)

            # film resistivity
            rho_j = np.maximum(8e6*np.exp(-0.1 * j), 2e6)

            # update film resistance
            R_film = np.maximum(R_film + dt * rho_j * Cv * (j-jMin), R_film0)
